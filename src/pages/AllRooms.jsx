  ...
  return (
    <label className="flex gap-3 items-center cursor-pointer mt-2 text-sm">
      <input
        ...
        name="sortOption"
        checked={selected}
        onChange={(label) => onChange(e.target.checked, label)}
      />
      <span className="font-light select-none">{label}</span>
    </label>
  );
...
const AllRooms = () => {
  ...
  return (
    <div className="flex flex-col-reverse lg:flex-row items-start justify-between pt-28 px-10 md:pt-35 md:px-16  lg:px-24 xl:px-32">
      <div>
        ...
        >
          ...
          <div className="px-5 pt-5">
            ...
            {priceRanges.map((range, index) => (
              <CheckBox key={index} label={`€ ${range} %range`} />
            ))}
          </div>
